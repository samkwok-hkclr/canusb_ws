    ...
    : Node("kinco_motor_node", options)
{
    ...
    if (node_id_ <= 0)
    {
        RCLCPP_ERROR(this->get_logger(), "Node ID Error!");
        return;
    }
    ...
    // Do not add namespace to the topics below
    pub_can_msg_ = this->create_publisher<VciCanObjMsg>("/can_req", can_qos);
    sub_can_req_ = this->create_subscription<VciCanObjMsg>("/can_msg", can_qos, std::bind(&KincoMotor::can_msg_cb, this, _1));
...
void KincoMotor::handle_tx1_pdo(const std::shared_ptr<VciCanObjMsg> msg)
{
    ...
    {
        float i_temp = (float) ((float) i_DEC / current_1A);
        motor_state_.actual_current = i_temp;
    }
}
...
void KincoMotor::handle_nmt_error_control(const std::shared_ptr<VciCanObjMsg> msg)
{
    (void) msg;
    const std::lock_guard<std::mutex> lock(this->mutex_);
